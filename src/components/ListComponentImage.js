...
const ListComponentImage = ({ imageData, isPopupUse }) => {
  const newDialogOpenType = "image";
  ...
  const list = imageData.map(component => (
    <ContextMenuTrigger
  ...
  const handleOpenComponentEdiotr = (e, d, target) => {
    ...
    const lookupData = imageData.filter(list => list.ID === choiceId);
  ...
  const callbackSetFlag = flag => {
    console.log("ListComponentImage 콜백");
    setEditDialogDatas({
      ...editDialogDatas,
      isOpenEditor: flag
    });
...
export default ListComponentImage;
